...
The canonical sync list lives at `client/practa/sync-manifest.json` — import it directly from the repo.

This file is **auto-generated** by `scripts/generate-sync-manifest.js`. It walks the import tree from the harness seed files AND scans every community Practa's `@/` imports to discover shared dependencies. It regenerates automatically whenever community Practa are synced (via `scripts/fetch-community-practa.js`), or you can regenerate manually:

```bash
node scripts/generate-sync-manifest.js
```
...
{
  "version": 2,
  "generated": "2026-02-25T...",
  "files": [
    { "path": "client/components/GlassCard.tsx", "category": "components" },
    { "path": "client/hooks/useTheme.ts", "category": "hooks" }
  ],
  "total": 19
}
```

Categories: `types`, `harness`, `components`, `context`, `lib`, `constants`, `hooks`.
