...
import { registerCdnRoutes } from "./cdn_routes";
...
function writeConfig(metadata: PractaMetadata): boolean {
  ...
  const orderedMetadata = {
    ...
    ...(metadata.estimatedDuration !== undefined && {
    ...
    try {
      ...
      if (!response.ok) {
        return res
          ...
          .json({ error: "Failed to check name availability" });
      }
  ...
  app.post("/api/practa/submit", async (req, res) => {
    ...
    try {
      ...
      if (!config) {
        return res
          .status(400)
          .json({ error: "Practa configuration not found" });
      }
...
`;
  ...
  app.get("/api/template/sync-status", async (req, res) => {
    try {
      ...
      try {
        ...
        if (appJsonResponse.ok) {
          ...
          latestTemplateVersion =
            remoteAppJson.expo?.version || localTemplateVersion;
        }
      } catch {}
      ...
      const hasNewerVersion =
        compareVersions(latestTemplateVersion, localTemplateVersion) > 0;
  ...
  app.post("/api/practa/reset-to-demo", async (req, res) => {
    try {
      ...
      const demoMetadataContent = fs.readFileSync(
        path.join(demoDir, "metadata.json"),
        "utf-8",
      );
      ...
      fs.writeFileSync(path.join(practaDir, "index.tsx"), demoIndexContent, {
        flag: "w",
      });
      fs.writeFileSync(
        path.join(practaDir, "metadata.json"),
        demoMetadataContent,
        { flag: "w" },
      );
  ...
  app.post("/api/template/update", async (req, res) => {
    try {
      ...
      for (const entry of zipEntries) {
        ...
        const shouldSkip = SKIP_PATTERNS.some((p) =>
  ...
  app.post("/api/template/publish", async (req, res) => {
    try {
      ...
      try {
        ...
        res.status(500).json({
          error:
            "Reset completed but Git push failed. You may need to push manually.",
          details:
            gitError instanceof Error ? gitError.message : "Unknown git error",
        });
      }
    } catch (error) {
  ...
  const APP_SYNC_CONFIG_PATH = path.resolve(
    process.cwd(),
    ".config/app-sync.config.json",
  );
  ...
  async function getGitHubAccessToken(
    ...
    try {
      ...
      if (githubConnectionSettings?.settings?.expires_at) {
        ...
        if (expiresAt > Date.now()) {
          return (
            ...
            githubConnectionSettings.settings.oauth?.credentials
              ?.access_token ||
            null
          );
        }
      ...
      if (connectionId) {
        ...
        const items = (
          data as { items?: Array<typeof githubConnectionSettings> }
        ).items;
      ...
      console.error(
        "Failed to get GitHub access token from Replit connector:",
        error,
      );
      return null;
    }
  ...
  app.post("/api/app-sync/sync", async (req, res) => {
    try {
      ...
      const manifestPath = path.resolve(
        projectRoot,
        ".config/.app-sync-manifest.json",
      );
      let previouslySyncedFiles: string[] = [];
      if (fs.existsSync(manifestPath)) {
        try {
          previouslySyncedFiles = JSON.parse(
      ...
      if (config.deleteStaleFiles) {
        const staleFiles = previouslySyncedFiles.filter(
          (f) => !currentSyncFiles.includes(f),
        );
        for (const staleFile of staleFiles) {
          try {
            ...
            console.error(
              `[App Sync] Failed to delete stale file ${staleFile}:`,
              deleteError,
            );
          }
        }
